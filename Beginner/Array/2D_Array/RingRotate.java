...
public class RingRotate {
    public static void main(String[] args) {
        ...
        System.out.println("Enter ring number (s):");
        int s = scn.nextInt();
        System.out.println("Enter rotations (k):");
        int k = scn.nextInt();

        System.out.println("Ring Rotate:");
        ringRotate(arr, s, k);
        print(arr);
        scn.close();
    }

    public static void ringRotate(int[][] arr, int s, int k){
        int[] oneDArray = fillOneDArray(arr, s);
        rotate(oneDArray, k);
        fillRing(arr, s, oneDArray);
    }
    ...
    public static void fillRing(int[][] arr, int s, int[] oneD){
        int minRow = s - 1;
        int minCol = s - 1;
        int maxRow = arr.length - s;
        int maxCol = arr[0].length - s;
